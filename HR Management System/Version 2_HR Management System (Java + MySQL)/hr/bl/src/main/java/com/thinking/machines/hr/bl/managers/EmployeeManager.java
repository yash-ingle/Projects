...
dsEmployee.setGender((gender=='M')?GENDER.MALE:GENDER.MALE);
...
Set<EmployeeInterface> ets=designationCodeWiseEmployeesMap.get(designation.getCode());
if(ets==null)
{
ets=new TreeSet<>();
ets.add(dsEmployee);
...
Set<EmployeeInterface> ets=designationCodeWiseEmployeesMap.get(oldDesignation.getCode());
if(ets!=null)
{
ets.remove(dsEmployee);
if(ets.size()==0) designationCodeWiseEmployeesMap.remove(oldDesignation.getCode());
}
...
{
ets.remove(dsEmployee);
if(ets.size()==0) designationCodeWiseEmployeesMap.remove(dsEmployee.getDesignation().getCode());
}
}catch(DAOException daoException)
{
BLException blException=new BLException();
...
employee.setGender((dsEmployee.getGender()=='M')?GENDER.MALE:GENDER.MALE);
...
{
BLException blException=new BLException();
blException.addException("designation","Invalid Designation : "+designationCode);
throw blException;
}
Set<EmployeeInterface> employees=new TreeSet<>();
EmployeeInterface employee;
