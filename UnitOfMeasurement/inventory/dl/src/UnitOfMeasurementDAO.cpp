...
{
pos=dataFile.tellg();
dataFile.read((char *)&_unitOfMeasurement,sizeof(unitOfMeasurement));
if(dataFile.fail()) break;
...
if(vTitle.length()>50)
{
throw DAOException(string("Inavlid unit of measurement : ")+vTitle);
}
Header header;
fstream dataFile(FILE_NAME,ios::in | ios::binary);
if(dataFile.fail())
...
if(dataFile.fail())
{
cout<<"Debug: Unable to open file"<<endl;
...
{
dataFile.close();
cout<<"Debug: Unable to read header"<<endl;
throw DAOException("Unit of measurement does not exist.");
}
if(header.numberOfRecords==0)
{
dataFile.close();
cout<<"Debug: numberOfRecords is zero"<<endl;
throw DAOException("Unit of measurement does not exist.");
}
_UnitOfMeasurement _unitOfMeasurement;
abc::IUnitOfMeasurement * unitOfMeasurement;
...
{
dataFile.close();
throw DAOException("No records added");
return 0;
}
dataFile.seekg(0,ios::beg);
dataFile.read((char *)&header,sizeof(Header));
